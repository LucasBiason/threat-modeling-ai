"""Unit tests for app.threat_analysis.llm.ollama_connection."""

from unittest.mock import MagicMock, patch

from app.threat_analysis.llm.ollama_connection import OllamaConnection


def _make_settings():
    s = MagicMock()
    s.ollama_model = "llama3"
    s.ollama_base_url = "[REDACTED]"
    return s


class TestOllamaConnectionProperties:
    def test_name(self):
        conn = OllamaConnection(_make_settings())
        assert conn.name == "Ollama"

    def test_init_stores_settings(self):
        settings = _make_settings()
        conn = OllamaConnection(settings)
        assert conn._settings is settings
        assert conn._llm is None

    def test_is_configured_always_true(self):
        conn = OllamaConnection(_make_settings())
        assert conn.is_configured() is True


class TestEnsureLLM:
    @patch("app.threat_analysis.llm.ollama_connection.ChatOllama")
    def test_success(self, mock_chat):
        mock_llm = MagicMock()
        mock_chat.return_value = mock_llm
        conn = OllamaConnection(_make_settings())
        result = conn._ensure_llm()
        assert result is mock_llm
        assert conn._llm is mock_llm

    @patch("app.threat_analysis.llm.ollama_connection.ChatOllama")
    def test_init_error(self, mock_chat):
        mock_chat.side_effect = RuntimeError("init failed")
        conn = OllamaConnection(_make_settings())
        result = conn._ensure_llm()
        assert result is None

    @patch("app.threat_analysis.llm.ollama_connection.ChatOllama")
    def test_cached(self, mock_chat):
        sentinel = MagicMock()
        conn = OllamaConnection(_make_settings())
        conn._llm = sentinel
        result = conn._ensure_llm()
        assert result is sentinel
        mock_chat.assert_not_called()


class TestParseJson:
    def test_valid_json_object(self):
        conn = OllamaConnection(_make_settings())
        result = conn._parse_json('{"key": "value"}')
        assert result == {"key": "value"}

    def test_valid_json_array(self):
        conn = OllamaConnection(_make_settings())
        result = conn._parse_json('[1, 2, 3]')
        assert result == [1, 2, 3]

    def test_json_in_code_block_stripped(self):
        conn = OllamaConnection(_make_settings())
        text = '```json\n{"x": 1}\n```'
        result = conn._parse_json(text)
        assert result == {"x": 1}

    def test_empty_response(self):
        conn = OllamaConnection(_make_settings())
        result = conn._parse_json("")
        assert result["error_type"] == "empty"
        assert result["service"] == "Ollama"

    def test_none_response(self):
        conn = OllamaConnection(_make_settings())
        result = conn._parse_json(None)
        assert result["error_type"] == "empty"

    def test_invalid_json(self):
        conn = OllamaConnection(_make_settings())
        result = conn._parse_json("no json here at all")
        assert result["error_type"] == "invalid_json"
        assert result["service"] == "Ollama"

    def test_json_with_surrounding_text(self):
        conn = OllamaConnection(_make_settings())
        result = conn._parse_json('Result: {"threats": []} end')
        assert result == {"threats": []}

    def test_object_before_array(self):
        """Ollama's _parse_json checks { before [ (no sorting), so object wins."""
        conn = OllamaConnection(_make_settings())
        result = conn._parse_json('[1, 2] then {"a": 1}')
        assert result == {"a": 1}

    def test_invalid_json_inside_brackets(self):
        conn = OllamaConnection(_make_settings())
        text = "{not: valid json}"
        result = conn._parse_json(text)
        assert result["error_type"] == "invalid_json"
